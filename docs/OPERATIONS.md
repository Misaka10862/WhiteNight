...
| QQ 隔了很久才回 | 先看 Mac 是否在睡眠（睡眠期间本机服务不运行）；若需 24h 在线接电源后运行 `scripts/keep_awake.sh start`；再看 `ollama ps` 是否有模型冷启动 |
| 记忆提取占用推理槽 | 已限长 512 token 且聊天优先取消；仍异常时查日志 `Ollama /api/chat` 长任务 |
