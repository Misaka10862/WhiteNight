...
最后更新：2026-08-15（第 7 轮，阶段 6）

## 当前阶段

- **阶段 6 · 完整 WebUI**：工作台已实现并通过构建/代理/API 验证，本轮收尾。
- 下一阶段：**阶段 7 · 后台服务与主动行为**。
...
- 报告：`docs/reports/phase6-verification.md`（随提交补齐）
...
1. launchd 后台服务：登录启动、异常拉起、健康检查、菜单栏状态入口。
2. 泊松主动消息调度：频率/静默时段/暂停期限/最近活动抑制。
3. 验证睡眠、唤醒、离线与网络恢复；关闭 WebUI 后服务仍运行。
4. 主动消息页面从占位变为真实配置界面；失败重试与过期消息不补发。
