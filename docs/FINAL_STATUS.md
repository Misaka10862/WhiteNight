...
| Routing and Delegation | The golden routing set meets the standards; Codex MCP real handshake; Hermes fails to secure without logging in |
| WebUI | 11 pages, session/memory/task/approval/permission/model/constraint/log/active message available; model page can switch Ollama resident policy (effective immediately and persistent) |
...
3. **Hermes real task**: `hermes model`/`hermes auth` Log in to Provider and run the real task link.
...
6. **Send active messages to QQ (optional)**: Change the local `config/whitenight.yaml`
   Change `proactive_sender` to `qq` to enable it (the target is the first owner_ids; default log).
